...
pub fn run(ctx: &Context) -> Result<(), AppError> {
    ...
    // `git worktree list` always places the main worktree first, so it is
    // guaranteed to be `worktrees[0]` for every repository layout (including
    // submodules and `core.worktree`). Sort the remaining linked worktrees by
    // name in place.
    if worktrees.len() > 1 {
